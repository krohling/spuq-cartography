...
def filter_dataset_items(dataset, tokenizer):
    def filter_fn(row):
        tokenized = tokenizer(
            row['question'],
            row["context"],
    ...
    ds_filtered = dataset.filter(filter_fn)
    print(f"Removed {len(dataset)-len(ds_filtered)} rows exceeding max length of {tokenizer.model_max_length} tokens.")
...
def process_datasets(dataset, tokenizer):
    ...
    train_dataset = filter_dataset_items(dataset['train'], tokenizer)
    train_dataset_featurized = train_dataset.map(
        lambda exs: prepare_train_dataset_qa(exs, tokenizer),
        batched=True,
        num_proc=1,
    ...
    if 'validation' in dataset:
        eval_dataset = filter_dataset_items(dataset['validation'], tokenizer)
        eval_dataset_featurized = eval_dataset.map(
            lambda exs: prepare_validation_dataset_qa(exs, tokenizer),
            batched=True,
